...
/**
 * VB.NET expression that regenerates a {{token}} value at run time, so a
 * re-run of the exported workflow submits fresh data rather than replaying the
 * recorded passport/email/phone (which de-duplicating apps reject).
 */
function vbGenerator(g: GeneratedValue): string | null {
  const n = Number(g.arg) || g.value.length;
  switch (g.kind) {
    case "digits": {
      if (n < 1 || n > 9) return null; // keep inside Int32 range
...
/**
 * Build the SetText `Text` attribute. Returns a VB expression in [...] when any
 * part of the value was generated, otherwise the plain literal.
 */
function xamlTextValue(
  text: string,
  generated: GeneratedValue[],
  template?: string,
): string {
  const hits = generated
    // Only this step's tokens - see csTextExpression for why.
    .filter((g) => (template ? template.includes(g.token) : true))
    .filter((g) => g.value && text.includes(g.value) && vbGenerator(g))
...
function renderStep(step: StepResult, generated: GeneratedValue[] = []): Rendered {
