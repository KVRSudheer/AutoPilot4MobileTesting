...
export async function uploadFarmApp(
  ...
  payload: { file?: { buffer: Uint8Array; filename: string }; url?: string; customId?: string },
): Promise<FarmApp> {
  if (!creds.username || !creds.accessKey) {
  ...
  if (creds.provider === "browserstack") {
    ...
    if (payload.file) {
      form.append("file", new Blob([payload.file.buffer as BlobPart]), payload.file.filename);
    } else if (payload.url) {
  ...
  if (creds.provider === "lambdatest") {
    ...
    const form = new FormData();
    if (payload.file) {
      form.append("appFile", new Blob([payload.file.buffer as BlobPart]), payload.file.filename);
    } else if (payload.url) {
      form.append("url", payload.url);
    }
    if (payload.customId) form.append("custom_id", payload.customId);
  ...
  const form = new FormData();
  form.append("payload", new Blob([payload.file.buffer as BlobPart]), payload.file.filename);
