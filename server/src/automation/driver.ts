...
export interface DeviceDriver {
  ...
  exists(selector: MobileSelector): Promise<boolean>; // immediate check
  // Poll until the element shows up (or the timeout elapses). Used by
  // "verify X is displayed" steps, which double as a wait for screens that
  // only appear after a network round-trip.
  waitForPresent(selector: MobileSelector, timeoutMs?: number): Promise<boolean>;
  screenSignature(): Promise<string>; // cheap fingerprint to detect screen changes
  // Dismiss the soft keyboard (native only); safe no-op elsewhere.
  dismissKeyboard(): Promise<void>;
  // GPS the app sees. setGeo returns what the device reports back, so the
  // caller can prove the location actually took effect.
  setGeoLocation(latitude: number, longitude: number): Promise<{ latitude: number; longitude: number } | null>;
  quit(): Promise<void>;
}
...
export class WebdriverDriver implements DeviceDriver {
  ...
  async setText(selector: MobileSelector, text: string): Promise<void> {
    ...
    // Native only: in a mobile browser, setValue already dispatches real
    // per-character key events, so auto-advancing inputs work as-is.
   ...
   */
  private async fixSegmentedInput(selector: MobileSelector, text: string): Promise<void> {
    if (!this.browser.keys) return;

    const landed = await this.getFieldValue(selector).catch(() => "");
    // Only act on the unambiguous signature: something landed, but less than
    // we sent. An empty read-back is "can't tell" - retyping there risks
    // doubling the value on a perfectly normal field.
    if (landed.length === 0 || landed.length >= text.length) return;

    try {
      await (await this.el(selector)).click(); // focus the first box
      await delay(150);
      // Clear whatever partially landed, then type one character at a time.
      await this.browser.keys("\uE003".repeat(text.length + 2)); // BACKSPACE
      await delay(150);
      for (const ch of text) {
        await this.browser.keys(ch);
        await delay(120); // let the app advance focus to the next box
      }
    } catch {
      // Leave the partial value rather than making things worse; the
      // orchestrator's verification will flag the step.
    }
  }
  ...
  async waitForPresent(selector: MobileSelector, timeoutMs = 15_000): Promise<boolean> {
  ...
  async setGeoLocation(
    ...
    // Read it back - the device is the only authority on whether it stuck.
